...
struct ForwardIterator *ForwardIterator_factory(void *data_ptr) {
  struct ForwardIterator *node =
      (struct ForwardIterator *)malloc(sizeof(struct ForwardIterator));
  node->next = NULL;
  node->data = data_ptr;
  return node;
}

void ForwardIterator_destructor(
    struct ForwardIterator *
        *to_destroy) { // WARNING ! Does not free data inside node
  free(*to_destroy);
  *to_destroy = NULL;
}
...
void *ForwardList_erase_after(ForwardList_t *forward_list,
                              struct ForwardIterator *first,
