...
enum class CarStatus { Pending, Repaired, Paid };
...
private:
  ...
  Client *m_owner{};
  CarStatus m_status = CarStatus::Pending;
...
public:
  ...
  Car(std::string brand, std::string model, std::string matriculation, Client *owner)
      : m_brand{std::move(brand)}, m_model{std::move(model)},
        m_matriculation{std::move(matriculation)}, m_owner{owner}, m_status{CarStatus::Pending} {};
  ...
  [[nodiscard]] const CarStatus &getStatus() const noexcept { return m_status; }
  ...
  void setStatusCode(int code) {
    switch (code) {
    case 1:
      m_status = CarStatus::Pending;
      break;
    case 2:
      m_status = CarStatus::Repaired;
      break;
    case 3:
      m_status = CarStatus::Paid;
      break;
    default:
      break;
    }
  ...
  int listCar() {
    ...
    for (Car *car : cars) {
      std::cout << "| " << car->getBrand() << " | " << car->getModel() << " | " << car->getMatriculation() << " | "
                << car->getOwner()->getName() << " | " << car->getStatusString() << " | " << i++ << " |" << std::endl;
    }
